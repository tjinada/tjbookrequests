// src/components/books/EnhancedSwipeableBookCard.js
...
const EnhancedSwipeableBookCard = ({ book, onRequest }) => {
  const theme = useTheme();
  const navigate = useNavigate();
  const [swipeDirection, setSwipeDirection] = useState(null);
  const [swipePercentage, setSwipePercentage] = useState(0);

  // Create an optimized book object
  const optimizedBook = {
    ...book
  };
  ...
  return (
    ...
    >
      ...
      >
        <EnhancedBookCard book={optimizedBook} />
      </Box>
    </Box>
  );
};

export default EnhancedSwipeableBookCard;